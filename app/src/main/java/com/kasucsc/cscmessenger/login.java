...
public class login extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        this.listener = new AuthStateListener() {
            public void onAuthStateChanged(@NonNull FirebaseAuth firebaseAuth) {
                ...
                Toast.makeText(login.this.getApplicationContext(), "logout " + user.getEmail(), Toast.LENGTH_SHORT).show();
            }
        };
        this.txtusername = findViewById(R.id.input_username);
        this.txtpassword = findViewById(R.id.input_password);
        this.buttonlogin = findViewById(R.id.btnlogin);
        this.textViewregister = findViewById(R.id.registerlink);
    ...
    private void userLogin() {
        ...
        this.pd.show();
        this.firebaseAuth.signInWithEmailAndPassword(username, password).addOnCompleteListener(this, new OnCompleteListener<AuthResult>() {
            public void onComplete(@NonNull Task<AuthResult> task) {
                login.this.pd.dismiss();
                if (task.isSuccessful()) {
                    login.this.databaseReference = FirebaseDatabase.getInstance().getReference().child("login");
                    Map<String, Object> map = new HashMap();
                    map.put("username", username);
                    map.put(EmailAuthProvider.PROVIDER_ID, password);
                    login.this.databaseReference.updateChildren(map);
                    login.this.pd.dismiss();
                    login.this.finish();
                    Toast.makeText(login.this, "Login Successfully", Toast.LENGTH_SHORT).show();
                    login.this.finish();
                    login.this.startActivity(new Intent(login.this.getApplicationContext(), studentprofile.class));
                    return;
                }
                login.this.pd.dismiss();
